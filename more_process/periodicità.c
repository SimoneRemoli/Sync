...
int main(int argc, char **argv)
{
	...
	semaforo_main = semget(IPC_PRIVATE, numero_processi, IPC_CREAT|IPC_EXCL|0666);
	...
	for(int i=1;i<numero_processi;i++)
		semctl(semaforo_main, i, SETVAL, 0);
...
redo1:
		...
		printf("\t Stiamo scrivendo sul file  = %s\n", file_names[indice + (indice + 1)]);
...
void handle(int unused)
{
	...
	if(indice == numero_processi)
	{
		...
		semctl(array_semaforico, indice - 1, SETVAL, 0); //blocco il precedente
