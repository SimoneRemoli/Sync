...
void handle(int unused)
{
	...
	printf("  -  Visualizzazione alternativa - [*] \n");
	...
	indexer = 0;
	contatore_appoggio = 0;
	contatore_nuovo = 0;



	//printf("----- APERTURA FILE [%d] ----- \n", indexer);


	while( EOF != fscanf(source_files[indexer], "%[^\n]\n", pointer))
	{
		//printf("Vaore di indexer = %d \n", indexer);
		printf(" \t  %s \n ", pointer);
		//printf("i =  %d \n", indexer);
		if(contatore_appoggio == 4)
		{
			indexer = indexer + 1;
			contatore_appoggio = 0;
			//printf("indexer = %d \n", indexer);
			//printf("Contatore acquisizioni = %d \n", contatore_acquisizioni); // se supera 4 index va in core dumped, perchè i file sono 4 

			if(indexer == numero_threads)
			{
				indexer = 0;
				contatore_appoggio = 0;
			}
		...
		{
			contatore_appoggio = contatore_appoggio + 1;
		}
